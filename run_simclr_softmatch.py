...
def train_simclr_softmatch_model(
    ...
    for epoch in epoch_iter:
        ...
        if (epoch + 1) % conf.eval_every == 0 or epoch == conf.pretrain_epochs - 1:
            ...
            save_path = f"{conf.model_saved_path}/softclr_{model_name}_epoch_{epoch + 1}_subset_{conf.softmatch_subset_ratio}_ts_{datetime.now().strftime('%H_%d/%m')}.pth" if save_model else None
